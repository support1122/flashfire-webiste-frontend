...
const Hero = ({setSignupFormVisibility, setCalendlyModalVisibility}) => {
  ...
  useEffect(() => {
    ...
    if (successMatrixRef.current) {
      observer.observe(successMatrixRef.current);
    }
  ...
  return (
    <>
      ...
      <section id="home" className="relative pb-4 top-12 h-screen bg-gradient-to-br from-orange-50 via-white to-red-50 overflow-hidden">
        {/* Background Effects */}
      ...
      {/* Success Matrix Section - Scroll to view */}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div 
            ...
            className={`max-w-7xl mx-auto text-center transition-all duration-800 ease-out opacity-100 transform translate-y-0 lg:opacity-0 lg:transform lg:translate-y-8 ${
              isSuccessMatrixVisible
                ? 'lg:opacity-100 lg:transform lg:translate-y-0'
                : ''
            }`}
          >
            <h2 className={`text-2xl sm:text-3xl md:text-4xl lg:text-4xl font-bold text-gray-900 mb-4 sm:mb-6 lg:mb-8 leading-tight transition-all duration-800 delay-100 opacity-100 transform translate-y-0 lg:opacity-0 lg:transform lg:translate-y-4 ${
              isSuccessMatrixVisible
                ? 'lg:opacity-100 lg:transform lg:translate-y-0'
                : ''
            }`}>
            ...
            {/* Subtitle */}
            <p className={`text-lg sm:text-xl md:text-xl lg:text-xl text-gray-600 mb-8 sm:mb-12 lg:mb-16 transition-all duration-800 delay-150 opacity-100 transform translate-y-0 lg:opacity-0 lg:transform lg:translate-y-4 ${
              isSuccessMatrixVisible
                ? 'lg:opacity-100 lg:transform lg:translate-y-0'
                : ''
            }`}>
            ...
            {/* Disclaimer */}
            <p className={`text-sm sm:text-base md:text-lg text-gray-500 italic transition-all duration-800 delay-400 opacity-100 transform translate-y-0 lg:opacity-0 lg:transform lg:translate-y-4 ${
              isSuccessMatrixVisible
                ? 'lg:opacity-100 lg:transform lg:translate-y-0'
                : ''
            }`}>
